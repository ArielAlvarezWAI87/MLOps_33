...
class RuleFitPredictor:
    ...
    def predict(self, X_raw: pd.DataFrame, log_to_mlflow: bool = True, 
        ...
        if log_to_mlflow:
            ...
            with mlflow.start_run(run_name=run_name) as run:
                ...
                mlflow.log_param("preprocessor_path", str(self.preprocessor_path))
                ...
                mlflow.log_metric("predictions_median", float(np.median(y_pred)))
                ...
                if log_model:
                    try:
                        ...
                        input_example = X_processed[:5]  # First 5 rows as example
                        ...
                        if register_model:
                            ...
                            print(f"   Model saved in run artifacts (not registered)")
                            print(f"   To register, use: register_model='your_model_name'")
    ...
    def register_current_model(self, model_name: str, run_id: str = None,
        ...
        try:
            ...
            if run_id is None:
                experiment = client.get_experiment_by_name("rulefit_predictions")
                ...
                if not runs:
                    raise ValueError("No runs found. Run predict() first with log_to_mlflow=True and log_model=True")
            ...
            print(f"   🔗 View in Models tab: http://localhost:5000/#/models/{model_name}")
    ...
    def transition_model_stage(self, model_name: str, version: str, 
        ...
        try:
            ...
            print(f"   🔗 View: http://localhost:5000/#/models/{model_name}/versions/{version}")
...
if __name__ == "__main__":
    ...
    X_new = df_engineered[[c for c in df_engineered.columns if c not in exclude_cols]]

    print(f"✅ Engineered feature matrix ready: {X_new.shape[0]} rows × {X_new.shape[1]} features")
